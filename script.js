...
const posts = [
    { 
        ...
        content: "简约设计并不是简单地去除元素，而是专注于核心功能。\n\n在构建这个个人主页时，我们使用了大量的留白和系统字体，这是为了让阅读体验回归本质。"
    },
    { 
        id: 1,
        title: "我的第一篇技术博客", 
        ...
        content: "今天我成功发布了自己的个人网页！通过这个项目，我学到了如何使用 HTML 构建结构，以及如何利用 CSS 进行布局。"
    }
];
...
function renderPostList() {
    ...
    posts.forEach((post, index) => {
        const itemHtml = `<div class="post-item">
...
function viewPostDetail(index) {
    ...
    detailContainer.innerHTML = `<p class="post-date">${post.date}</p>
        <h1>${post.title}</h1>
...
function renderPublications() {
    console.log("正在渲染，当前文献总数:", publications.length);
    const pubList = document.getElementById('pub-list');
    if (!pubList) return;
    pubList.innerHTML = ''; 
    
    publications.forEach((pub, index) => {
        // 动态处理卷号、期号和页码，如果不存在则不显示
        const volInfo = pub.volume ? `, ${pub.volume}` : '';
        const numInfo = pub.number ? `(${pub.number})` : '';
        const pageInfo = pub.pages ? `, ${pub.pages}` : '';
        ...
        const html = `
            <div class="pub-item-academic">
                ...
                <p class="citation">
                    ...
                    <em>${pub.journal}</em>${volInfo}${numInfo}${pageInfo}.
                </p>
                <div class="pub-links">
...
document.addEventListener('DOMContentLoaded', () => {
    console.log("主页脚本初始化成功。文献总数:", publications.length);
    showSection('about');
});
